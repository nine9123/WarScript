...
namespace WarScript.Expression
{
    public class ExpressionReader
    {
        ...
        public static IExpression ReadExpression(TokensStack tokens)
        {
            var reader = new ExpressionReader(tokens);
            return reader.ReadExpression();
        }

        public static IExpression ReadExpression(ExpressionReader reader)
        {
            return ReadExpression(reader.Tokens);
        }
        ...
        private IExpression ReadExpression()
        {
                    ...
                    default:
                        ...
                        switch (token.Type)
                        {
                            case TokenType.Numeric:
                                operand = new NumericValue(double.Parse(value));
                                break;
                            case TokenType.Logical:
                                operand = new LogicalValue(bool.Parse(value));
                                break;
                            case TokenType.Text:
                            ...
                            case TokenType.GroupDivider when token.Value == "{":
                                ...
                                break;
                            case TokenType.Null:
                                operand = NullValue.Instance;
                                break;
                            case TokenType.This:
                                operand = ThisValue.Instance;
                                break;
            ...
            return _operands.Count == 0 ? NullValue.Instance : _operands.Pop();
        }
        ...
        private void ApplyTopOperator()
        {
            ...
            if (op.IsBinary())
            {
                ...
                _operands.Push(op.ToBinaryExpression(right, left));
            }
            else
            {
                _operands.Push(op.ToUnaryExpression(left));
            }
        }
        ...
        private ClassExpression ReadClassInstance(Token.Token token)
        {
            ...
            if (Tokens.PeekSameLine(TokenType.GroupDivider, "["))
            {
                ...
                {
                    properties.Add(ReadExpression(this));
                    if (Tokens.PeekSameLine(TokenType.GroupDivider, ","))
                        Tokens.Next();
                }
            ...
            return new ClassExpression(token.Value, properties);
        }
        ...
        private FunctionExpression ReadFunctionInvocation(Token.Token token)
        {
            ...
            if (Tokens.PeekSameLine(TokenType.GroupDivider, "["))
            {
                ...
                while (!Tokens.PeekSameLine(TokenType.GroupDivider, "]"))
                {
                    arguments.Add(ReadExpression(this));
                    if (Tokens.PeekSameLine(TokenType.GroupDivider, ","))
                        Tokens.Next();
                }
            ...
            return new FunctionExpression(token.Value, arguments);
        }
        ...
        private ArrayExpression ReadArrayInstance()
        {
            ...
            while (!Tokens.PeekSameLine(TokenType.GroupDivider, "}"))
            {
                values.Add(ReadExpression(this));
                if (Tokens.PeekSameLine(TokenType.GroupDivider, ","))
                    Tokens.Next();
            }
        ...
        {
            var array = new VariableExpression(token.Value);
            Tokens.Next(TokenType.GroupDivider, "{");
            var arrayIndex = ReadExpression(this);
            Tokens.Next(TokenType.GroupDivider, "}");
